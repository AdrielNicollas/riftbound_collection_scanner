...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a small TensorFlow classifier for Riftbound domain symbols.",
    )
...
def main():
    ...
    keras_path = args.output_dir / "riftbound_domain_classifier.keras"
    model.save(keras_path)

    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    converter.optimizations = [tf.lite.Optimize.DEFAULT]
    tflite_model = converter.convert()
    tflite_path = args.output_dir / "riftbound_domain_classifier.tflite"
