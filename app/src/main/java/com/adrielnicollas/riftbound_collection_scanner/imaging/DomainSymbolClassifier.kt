...
class DomainSymbolClassifier(context: Context) : Closeable {
    ...
    fun classify(bitmap: Bitmap): DomainPrediction? {
        ...
        return DomainPrediction(
            domain = labels.getOrNull(best.index).orEmpty(),
            confidence = best.value,
        ).takeIf { it.domain.isNotBlank() && it.confidence >= MIN_CONFIDENCE }
    }
    ...
    companion object {
        private const val MODEL_FILE = "riftbound_domain_classifier.tflite"
